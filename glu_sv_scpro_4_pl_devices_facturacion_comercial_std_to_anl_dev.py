...
#formateamos la fecha
fecha_formateada = fecha_anterior.strftime('%Y%m%d')
df = spark.read.options(encoding='UTF-8').parquet("s3a://s3-sv-std-dev-scpro/4-pl-devices/facturacion-comercial/DatePartKey=" + fecha_formateada)
df.write.format("parquet").mode("overwrite").save("s3a://s3-sv-anl-dev-scpro/4-pl-devices/facturacion-comercial/")
job.commit()